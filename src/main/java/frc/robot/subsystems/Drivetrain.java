...
// ! Navx Dönüşten sonra ters gidiyor
...
public class Drivetrain extends SubsystemBase {
  ...
  public void DriveArcade() {
    ...
    if (Math.abs(turn) > 0.05) { 
        ...
        isTurning = false;
        targetAngle = navx.getYaw(); // Kullanıcı dönüşü bıraktığında hedef açıyı kaydet
    }

    // Sadece ileri veya geri giderken düzeltme uygula
    if (!isTurning && Math.abs(forward) > 0.05) {
        double currentYaw = navx.getYaw();
        double error = targetAngle - currentYaw;
        double correction = error * 0.07; // Hata düzeltme katsayısı artırıldı (0.07)
        correction = Math.max(-0.4, Math.min(0.4, correction)); // Düzeltme sınırı artırıldı (-0.4, 0.4)
